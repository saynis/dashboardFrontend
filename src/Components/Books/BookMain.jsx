...
const BooksMain = () => {
  ...
  const fetchBooks = async () => {
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/books/all`);
  ...
  const handleSubmit = async (event) => {
    ...
    const {data} = await axios.post(`${import.meta.env.VITE_API_URL}/api/books/new`, formValues)
    setBooks([...books , data])
    fetchBooks()
    handleClose()
  ...
  const handleUpdate = async (event) => {
    event.preventDefault();
    const {data} = await axios.put(`${import.meta.env.VITE_API_URL}/api/books/update/${updateId}`, formValues)
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/books/delete/${id}`);
      fetchBooks();
    } catch (error) {
